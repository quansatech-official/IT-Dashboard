...
function CustomerCard({ customer, reload }) {
  ...
  const suggestionOptions = useMemo(() => {
    const sourceText = normalizeText(customer.name);
    if (!sourceText) return [];
    const tokens = sourceText.split(" ").filter((token) => token.length > 2);
    return mergeOptions
      .map((option) => {
        const nameText = normalizeText(option.name);
        if (!nameText) return null;
        const shortCodeText = normalizeText(option.short_code || option.shortCode || "");
        let score = 0;
        if (sourceText.includes(nameText)) {
          score += 100 + nameText.length;
        }
        if (shortCodeText && sourceText.includes(shortCodeText)) {
          score += 80 + shortCodeText.length;
        }
        if (!score) {
          tokens.forEach((token) => {
            if (nameText.includes(token)) score += 5;
            if (shortCodeText && shortCodeText.includes(token)) score += 8;
          });
        }
              ...
              {mergeOpen ? (
                <div className="absolute left-0 mt-2 w-52 rounded-xl border border-slate-200 bg-white shadow-lg z-20">
                  {suggestionOptions.length ? (
                    <>
                      <div className="px-3 pt-2 text-[10px] uppercase tracking-wide text-slate-400">
                        Vorschläge
                      </div>
                      <div className="pb-2">
                  ...
                  <div className="p-2">
                    <input
                      ...
                      onKeyDown={(event) => {
                        ...
                        event.preventDefault();
                        const options = filteredMergeOptions.length
                          ? filteredMergeOptions
                          : mergeOptions;
                        if (options.length) {
                          setMergeOpen(false);
                          onMerge?.(options[0].id);
                          return;
                        }
                        if (trimmedMergeQuery) {
